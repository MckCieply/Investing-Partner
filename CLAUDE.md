...
- **Tryb Review** (`workflow_dispatch` z `mode: review`): ten sam łańcuch 5 agentów, ale ocenia **posiadane** pozycje zamiast szukać nowych — czy pierwotna teza/katalizator zadziałały → werdykt TRZYMAJ/DOKŁADAJ/REDUKUJ/ZAMKNIJ. Tickery z `review_tickers.txt` w repo danych (pusty = cały `holdings.json`; nigdy jako input workflowu — inputy są widoczne w publicznym repo). Wynik `reports/review-<data>.md`, bez wpisów do `recommendations.csv`/`scout_tickers.csv`. Szczegóły: sekcja „Tryb Review" w SKILL.md.
